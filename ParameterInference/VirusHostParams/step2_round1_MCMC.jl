# Code by Marian Dominguez-Mirazo and David Demory, 2024
# Usage: julia step2_round1_MCMC_vir.jl dataset_folder dataset_id chain_id
# This script takes previously calculated (see step1) initial estimates of virus-host parameters and informs priors for MCMC parameter estimation.
...
@everywhere path = "step2_MCMC/round1/"*dataid
...
# Load initial paramter estimates
...
@everywhere begin
    ...
    ϕ_estimate = pars_estimate[3,1];
    β_estimate = pars_estimate[4,1];
    η_estimate = pars_estimate[5,1];
    cv_estimate = pars_estimate[6,1];
    # Size control the CV estimate for running time purposes
    if cv_estimate < 0.2
	cv_estimate = 0.2
    end
...
@everywhere d_ϕ = truncated(Normal(log10(ϕ_estimate),2),-11,-5)
@everywhere d_β = truncated(LogNormal(for_mu(β_estimate,β_estimate*2),for_sigma(β_estimate,β_estimate*2)),1,1000)
@everywhere d_η = truncated(LogNormal(for_mu(η_estimate,η_estimate*2),for_sigma(η_estimate,η_estimate*2)),1/12,60/10)
@everywhere d_CV = truncated(LogNormal(for_mu(cv_estimate,cv_estimate+0.3),for_sigma(cv_estimate,cv_estimate+0.3)),0.03,1) 
...
writedlm(path*"/viruschain"*id*"_"*runid*".csv",chainarray,",");
