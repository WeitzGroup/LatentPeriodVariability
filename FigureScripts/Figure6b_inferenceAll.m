...
for j =1:size(dataids,1)
    ...
    % Aesthetics
    ylabel('Estimated T, hr','interpreter','latex','FontSize',20);
    ylim([0,1.5*true_T]);
    xlim([0,0.55]);
    xticks(0:0.1:0.5);
    box off;
    ...
    % Aesthetics
    ylabel('Estimated CV','interpreter','latex','FontSize',20);
    xlim([0,0.55]);
    ylim([0,0.55]);
    xticks(0:0.1:0.5);
    yticks(0:0.1:0.5);
...
nexttile(5);
xlabel('Original CV','interpreter','latex','FontSize',20);
...
%% Save figure
saveas(gcf,'../Figures/Figure6b.svg');