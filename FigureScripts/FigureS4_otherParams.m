...
for j =1:size(dataids,1)
    ...
    % Aesthetics
    ylabel({'Estimated'; 'adsorption rate ($\phi$)';'ml/(CFU$\times$hr)'},'interpreter','latex','FontSize',19);
    ...
    % Aesthetics
    ylabel('Estimated Burst Size ($\beta$)','interpreter','latex','FontSize',19);
    ylim([0,2*true_beta]);
    xlim([0,0.55]);
    xticks(0:0.1:0.5);
    box off;
...
nexttile(5);
xlabel('Original CV','interpreter','latex','FontSize',19);
...
%% Save figure
saveas(gcf,'../Figures/FigureS4.svg');